...
blue_pic = cv.merge([blank,image_green,image_red])
red_pic = cv.merge([image_blue,image_green,blank])
green_pic = cv.merge([image_blue,blank,image_red])

identifier_blue = cv.bitwise_and(image_grey,image_blue)
